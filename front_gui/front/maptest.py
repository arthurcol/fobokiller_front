...
def read_base_file():
    ...
    station_stats['pct_precip_change'] = (
        100 * station_stats.slope_total_precip_pct.round(3))
...
metric_descs = {
    'slope_max_temp':
    'Annual change (degrees Celsius) in average daily high temp',
    'slope_min_temp':
    'Annual change (degrees Celsius) in average daily low temp',
...
name_in_annual_data = {
    'slope_max_temp': 'max_temp_c',
    'slope_min_temp': 'min_temp_c',
    'pct_precip_change': 'precip_mm',
}

graph_descriptions = {
    'max_temp_c': 'Average daily high temperature (Celsius)',
...
def make_city_graphs(allow_output_mutation=True):
    ...
    def make_one_city_map_graphs(annual_this_city, city_name,
                                 annual_data_field):
        graph = alt.Chart(annual_this_city,
                          title=city_name).mark_line().encode(
                              alt.X('year',
                                    scale=alt.Scale(zero=False),
                                    axis=alt.Axis(format="d")),
                              alt.Y(annual_data_field,
                                    scale=alt.Scale(zero=False)),
                          )
        return graph

    def make_one_city_standalone_graphs(annual_this_city, city_name,
                                        annual_data_field):
        graph = alt.Chart(annual_this_city,
                          title=city_name).mark_point().encode(
                              alt.X('year',
                                    scale=alt.Scale(zero=False),
                                    axis=alt.Axis(format="d")),
                              alt.Y(annual_data_field,
        ...
        return graph + graph.transform_regression(
            'year', annual_data_field).mark_line()

    out = {'for_map': {}, 'standalone': {}}
    for _, city in station_stats.iterrows():
        annual_this_city = annual_stats.loc[annual_stats.station_id ==
                                            city.station_id]
        city_name = city.municipality
        station_id = city.station_id
        out['for_map'][station_id] = {
            summary_stat: make_one_city_map_graphs(annual_this_city, city_name,
                                                   annual_stat)
            for summary_stat, annual_stat in name_in_annual_data.items()
        }
        out['standalone'][city_name] = {
            annual_stat:
            make_one_city_standalone_graphs(annual_this_city, city_name,
                                            annual_stat)
            for summary_stat, annual_stat in name_in_annual_data.items()
        }
    return out
...
def make_map(field_to_color_by):
    ...
    colormap = linear.RdYlBu_08.scale(
        station_stats[field_to_color_by].quantile(0.05),
    ...
    for _, city in station_stats.iterrows():
        ...
        folium.CircleMarker(
            ...
            popup=folium.Popup().add_child(
                folium.features.VegaLite(city_graph))).add_to(main_map)
    return main_map
...
city_name = st.selectbox(
    "City",
    sorted(station_stats.loc[station_stats.region ==
                             region].municipality.unique()))
